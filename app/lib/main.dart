...
class _TheOnlyShellState extends State<TheOnlyShell>{
 ...
 @override Widget build(BuildContext context){final visible=settings.visibleSections;if(!visible.contains(selected))selected=visible.first;return FocusTraversalGroup(child:Scaffold(appBar:AppBar(title:const Text('The Only'),actions:[IconButton(key:const Key('library-button'),tooltip:'Library',onPressed:openLibrary,icon:const Icon(Icons.video_library)),IconButton(key:const Key('settings-button'),tooltip:'Settings',onPressed:openSettings,icon:const Icon(Icons.settings))]),body:sectionBody(),bottomNavigationBar:NavigationBar(selectedIndex:visible.indexOf(selected),onDestinationSelected:(i)=>setState(()=>selected=visible[i]),destinations:[for(final s in visible)NavigationDestination(icon:Icon(switch(s){SectionId.cinema=>Icons.movie_outlined,SectionId.liveTv=>Icons.live_tv_outlined,SectionId.sources=>Icons.source_outlined,SectionId.resolvers=>Icons.link_outlined,SectionId.tools=>Icons.tune_outlined,SectionId.optional=>Icons.monitor_heart_outlined}),label:labels[s]!) ])));}
}
