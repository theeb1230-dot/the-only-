...
///
/// The provider deliberately returns only records with a Creative Commons or
/// public-domain license URL, and it re-checks that license on the metadata
/// response before exposing a playable file. This keeps remote discovery
/// useful without turning the app into an unrestricted scraper.
final class InternetArchiveProvider implements MediaProvider {
  ...
  Future<List<MediaItem>> search(String query) async {
    ...
    for (final raw in docs) {
      ...
      if (identifier == null || title == null || !_isOpenLicense(license)) {
        continue;
      }
      items.add(
        MediaItem(
  ...
  Future<ProviderResult> sourcesFor(MediaItem item) async {
    ...
    for (final raw in files) {
      ...
      if (format.isNotEmpty && !format.contains('mpeg4') && !format.contains('h.264')) {
        continue;
      }
      final url = Uri(
        scheme: 'https',
