...
class _ResolversScreenState extends State<ResolversScreen> {
  ...
  Future<void> _resolve() async {
    ...
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) { setState(() { _supported = false; _results = const []; _error = 'أدخل رابطًا صالحًا'; }); return; }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _results = results; if (results.isEmpty) _error = 'لم يُرجع المحلل أي مصادر تشغيل'; });
    } catch (_) {
  ...
  @override Widget build(BuildContext context) => ListView(key: const Key('resolvers-screen'), padding: const EdgeInsets.all(16), children: [
    TextField(key: const Key('resolver-uri'), controller: _input, keyboardType: TextInputType.url, decoration: const InputDecoration(labelText: 'الرابط المراد تحليله')),
    const SizedBox(height: 8), FilledButton.icon(key: const Key('resolver-run'), onPressed: _busy ? null : _resolve, icon: const Icon(Icons.link), label: const Text('تحليل')),
    if (_busy) const LinearProgressIndicator(key: Key('resolver-loading')),
    if (_supported != null) Text(_supported! ? 'مدعوم' : 'غير مدعوم', key: const Key('resolver-support')),
    ...
    for (var i = 0; i < _results.length; i++) ListTile(key: Key('resolver-result-$i'), title: Text(_results[i].quality ?? _results[i].protocol.name.toUpperCase()), subtitle: Text('${_results[i].providerId} • ${_results[i].protocol.name}'), trailing: Wrap(spacing: 8, children: [
      FilledButton(key: Key('resolver-watch-$i'), onPressed: widget.onWatch == null ? null : () => widget.onWatch!(_results[i]), child: const Text('مشاهدة')),
      if (isDownloadable(_results[i])) OutlinedButton(key: Key('resolver-download-$i'), onPressed: widget.onDownload == null ? null : () => widget.onDownload!(_results[i]), child: const Text('تنزيل')),
    ])),
  ]);
}
