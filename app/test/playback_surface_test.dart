...
void main() {
  test('HLS MP4 and DASH route to native playback while embed is isolated', () {
    for (final protocol in [
      ...
      expect(
        playbackSurfaceFor(source(protocol, 'https://media.example/video')),
        PlaybackSurface.native,
      );
    }
    expect(
      playbackSurfaceFor(
        source(StreamProtocol.embed, 'https://embed.example/watch'),
      ),
      PlaybackSurface.legalEmbed,
    );
  });

  testWidgets('embed product route fails closed when no legal host is allowlisted',
      (tester) async {
    final embed =
        source(StreamProtocol.embed, 'https://unapproved.example/watch');
    await tester.pumpWidget(
      MaterialApp(home: PlaybackScreen(source: embed, title: 'Embed')),
    );

    expect(find.byType(LegalEmbedScreen), findsOneWidget);
    expect(find.byKey(const Key('embed-error')), findsOneWidget);
    expect(
      find.text('Embed blocked by the legal navigation policy.'),
      findsOneWidget,
    );
