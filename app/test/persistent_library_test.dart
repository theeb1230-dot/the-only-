...
void main() {
  ...
  test('persists favorites history and downloads across repository recreation', () async {
    ...
    const item = MediaItem(id: 'movie-1', title: 'Persistent Movie', kind: MediaKind.movie);
    ...
    await downloads.enqueue(DownloadJob(id: 'download-1', source: source, state: DownloadState.queued));

    final recreated = PersistentLibraryStore(await SharedPreferences.getInstance());
  ...
  test('deduplicates favorites, history and download jobs by stable id', () async {
    ...
    await history.save(HistoryEntry(item: first, position: Duration.zero, updatedAt: DateTime.utc(2026)));
    await history.save(HistoryEntry(item: replacement, position: const Duration(seconds: 9), updatedAt: DateTime.utc(2026, 1, 2)));
    await downloads.enqueue(DownloadJob(id: 'same-job', source: source, state: DownloadState.queued));
    await downloads.enqueue(DownloadJob(id: 'same-job', source: source, state: DownloadState.paused));
