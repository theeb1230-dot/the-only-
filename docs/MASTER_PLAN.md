...
| TO-11 | Cinema details/list/seasons/episodes where applicable | DONE | production Cinema exposes a visible details state after opening the legal catalog result, including media kind and discovered playback-source count before Watch/Download actions; runtime product-path test proves search -> result -> details -> source actions. The current authorized catalog contains a movie only, so seasons/episodes are not fabricated and remain not applicable until a legal series provider is registered. |
...
| TO-16 | Persistent legal Downloads manager | DONE | production Library starts/retries/cancels legal direct-MP4 transfers into app-owned storage with visible progress; lifecycle/path/error persist, interrupted jobs recover honestly after restart, missing files fail closed, redirects are revalidated, partial files are cleaned, and integration tests transfer real bytes through HttpClient |
...
| TO-25 | Player core HLS/MP4/DASH with platform-safe behavior | IN_PROGRESS | native VideoPlayerController initialization/play, bounded retry/timeouts, runtime error/buffering feedback, lifecycle pause/resume/back/dispose and stale-initialization fencing are implemented and CI/build green; physical/device playback proof for MP4/HLS/DASH remains under TO-34 |
| TO-26 | Legal embed/WebView last-resort fallback + host/navigation policy | DONE | BUILD-1.39 on exact main `b0590674afd9cbe051e7dfa5579d281752be5277`: in-app `LegalEmbedScreen`, explicit host allowlist, fail-closed initial/navigation policy, JavaScript disabled by default, no external-browser path, CI #277 green, Release Build #236 green; physical-device proof remains tracked under TO-28/29/30/34 |
...
| TO-29 | Android TV 10-foot UX, D-pad/focus/LEANBACK/launcher | IN_PROGRESS | independent TV APK + LEANBACK build verification exists; TvFocusScope D-pad traversal is wired into the production shell with deterministic RTL focus tests; physical TV/remote UX proof remains DEVICE_REQUIRED_PENDING |
...
| TO-32 | Functional integration/smoke suite for all six interfaces | DONE | PR #68 merged at main `9730c97604bdf4aa3e93b1b32d16cc31554134b8`; deterministic six-interface smoke covers Cinema, Live TV, Sources, Resolvers, Tools/Providers and the gated sixth interface with visible loading/error/empty/results/actions/back/state paths; this is CI/integration evidence, not DEVICE evidence |
| TO-33 | LIVE verification suite using legal/authorized sources | DONE | dedicated LIVE Verification workflow is separate from deterministic CI and has passed on current product history using only legal/authorized sample sources; failures remain honest and do not convert DEVICE requirements into PASS |
...
| TO-36 | Accessibility/localization/RTL/TV focus consistency | IN_PROGRESS | app shell is RTL; semantic loading/error states exist; deterministic D-pad focus traversal is wired and tested, while physical TV focus and broader platform accessibility evidence remain pending |
